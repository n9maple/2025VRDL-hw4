...
def pad_input(input_, img_multiple_of=8):
    ...
    H, W = ((height + img_multiple_of) // img_multiple_of) * img_multiple_of, (
        (width + img_multiple_of) // img_multiple_of
    ) * img_multiple_of
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt_name", type=str, default="model.ckpt", help="checkpoint save path"
    )
    ...
    with torch.no_grad():
        for [clean_name], degrad_patch in tqdm(testloader):
            ...
            if opt.tile is False:
                ...
                restored = tile_eval(
                    net, degrad_patch, tile=opt.tile_size, tile_overlap=opt.tile_overlap
                )
