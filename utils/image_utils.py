...
def slice_image2patches(image, patch_size=64, overlap=0):
    ...
    image_padding = np.pad(
        image, ((overlap, overlap), (overlap, overlap), (0, 0)), mode="edge"
    )
    for h in range(H // patch_size):
        for w in range(W // patch_size):
            ...
            patches.append(
                np.expand_dims(
                    image_padding[idx_h[0] : idx_h[1], idx_w[0] : idx_w[1], :], axis=0
                )
            )
    ...
    for h in range(H // patch_size):
        for w in range(W // patch_size):
            image[
                ...
                idx, overlap : patch_size + overlap, overlap : patch_size + overlap, :
            ]
            idx += 1
    return image
