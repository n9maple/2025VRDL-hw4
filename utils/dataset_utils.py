...
class HW4PromptIRDataset(Dataset):
    def __init__(self, root_dir, patch_size=256):
        ...
        for fname in os.listdir(self.clean_dir):
            if fname.startswith("rain_clean-") or fname.startswith("snow_clean-"):
                if fname.startswith("rain_clean-"):
                    task = "derain"
                    degraded_fname = fname.replace("rain_clean-", "rain-").replace(
                        ".png", ".png"
                    )
                else:
                    task = "desnow"
                    degraded_fname = fname.replace("snow_clean-", "snow-").replace(
                        ".png", ".png"
                    )
                degraded_path = os.path.join(self.degraded_dir, degraded_fname)
                    ...
                    self.pair_list.append(
                        {
                            ...
                            "clean": clean_path,
                            "task": task,
                            "name": degraded_fname[:-4],
                        }
                    )
                else:
                    print(f"Warning: {degraded_path} not found for {clean_path}")

        self.toTensor = ToTensor()
        self.crop_transform = Compose([RandomCrop(self.patch_size)])

    def __len__(self):
        return len(self.pair_list)

    def __getitem__(self, idx):
        sample = self.pair_list[idx]
        degraded_img = Image.open(sample["degraded"]).convert("RGB")
        clean_img = Image.open(sample["clean"]).convert("RGB")

        # Convert to tensor then random crop
        degraded_img = self.toTensor(degraded_img)
        clean_img = self.toTensor(clean_img)
        stacked = torch.cat(
            [degraded_img.unsqueeze(0), clean_img.unsqueeze(0)], dim=0
        )  # shape (2,3,H,W)
        stacked = self.crop_transform(stacked)
        degraded_img = stacked[0]
        clean_img = stacked[1]

        # 任務名稱轉成數字ID，方便 promptIR 輸入
        task2id = {
            "derain": 0,
            "desnow": 1,
        }  # 你可自定義，需對應 promptIR 的 prompt/token系統
        de_id = task2id[sample["task"]]
        clean_name = sample["name"]

        return [clean_name, de_id], degraded_img, clean_img
...
class PromptTrainDataset(Dataset):
    ...
    def _init_clean_ids(self):
        ...
        clean_ids += [
            self.args.denoise_dir + id_ for id_ in name_list if id_.strip() in temp_ids
        ]
    ...
    def _crop_patch(self, img_1, img_2):
        ...
        patch_1 = img_1[
            ind_H : ind_H + self.args.patch_size, ind_W : ind_W + self.args.patch_size
        ]
        patch_2 = img_2[
            ind_H : ind_H + self.args.patch_size, ind_W : ind_W + self.args.patch_size
        ]
    ...
    def _get_gt_name(self, rainy_name):
        # 取出目錄的前綴（rainy之前的全部字串）
        prefix = rainy_name.split("degraded")[0]
        # 取出檔名
        filename = rainy_name.split("/")[-1]  # e.g., rain-X.png or snow-X.png

        if filename.startswith("rain-"):
            gt_filename = filename.replace("rain-", "rain_clean-")
        elif filename.startswith("snow-"):
            gt_filename = filename.replace("snow-", "snow_clean-")
        else:
            raise ValueError("檔名必須以 rain- 或 snow- 開頭！目前為：" + filename)
        gt_name = prefix + "clean/" + gt_filename
        return gt_name
