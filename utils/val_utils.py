...
def compute_psnr_ssim(recoverd, clean):
    ...
    for i in range(recoverd.shape[0]):
        # 取得該圖最短邊
        min_side = min(clean[i].shape[0], clean[i].shape[1])
        win_size = (
            7 if min_side >= 7 else min_side if min_side % 2 == 1 else min_side - 1
        )
        # 兼容新版skimage
        try:
            ssim += structural_similarity(
                clean[i], recoverd[i], data_range=1, win_size=win_size, channel_axis=-1
            )
        except TypeError:
            # 舊版skimage fallback
            ssim += structural_similarity(
